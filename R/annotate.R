...
annotate_variants <- function(x, outpath, outfile,
  ...
  if(build == "hg19"){
    if(missing(protocols_g))	
      protocols_g = "refGene,knownGene"
    if(missing(protocols_f))	
      protocols_f = "avsift,ljb26_all,cg69,cosmic70,cosmic72,cosmic72conf,esp6500siv2_all,snp138,nci60,clinvar_20150330,cytoBand,tcga_nb_v1"
  }
  ...
  ## --- outfile is w/o extension (will have multianno later)
  if(missing(outfile))
  ...
  cmd = paste("perl", file.path(annovarPath, "table_annovar.pl"), 
              ## input, db_path, get other columsns, put . for NA, remove tmp files, build
              input, file.path(db_path), "-otherinfo -nastring . -remove -buildver", build,
...
annotate_mutect <- function(x, odir, oprefix,
  ...
  annedMutect = tmp$data
  annedName <- tmp$outfile
  
  ## REPLACE NA with N/A
  ## all columns are lower case, to avoid errors downstream
  annedMutect[is.na(annedMutect[, "gene.refgene"]), "gene.refgene"] <- "N/A"
  annedMutect[is.na(annedMutect[, "gene.knowngene"]), "gene.knowngene"] <- "N/A"
  
  ## get Entrez gene ids
  if(tolower(organism) == "human"){
    require(org.Hs.eg.db)
    gene_ids <- sapply(AnnotationDbi::mget(annedMutect[, "gene.refgene"],
                                           env =  org.Hs.egALIAS2EG, ifnotfound = as.list(NA)),
                       function(x) paste(x, collapse = ";"))
  }
  if(tolower(organism) == "mouse"){
    require(org.Mm.eg.db)
    gene_ids <- sapply(AnnotationDbi::mget(annedMutect[, "gene"],
                                           env =  org.Mm.egALIAS2EG, ifnotfound = as.list(NA)),
                       function(x) paste(x, collapse = ";"))
  }
  annedMutect <- cbind(annedMutect, entrez_gene_id = gene_ids)
  #geneIDs <- unlist(mclapply(annedMutect[, "Gene"], symbolEntrez,
  #    to = "entrez", ensembl = ensembl, mc.cores = cores))
  #geneIDs[geneIDs == ""] <- "No"
  #annedMutect <- cbind(annedMutect, Entrez_Gene_Id = geneIDs)
  ## get unipros
  if(unipro){
    ensembl <- getEnsemblMart(dataset)
    interMap <- read.delim(interpro, sep = "\t", header = FALSE, as.is = TRUE)
    unipros <- try(t(sapply(annedMutect[, "aachange"], aaChange2Unipro,
                            filters = filters, att = att, interMap = interMap, ensembl = ensembl)),
                   silent = TRUE)
    if(class(unipros) != "try-error"){
      annedMutect <- cbind(as.matrix(annedMutect), unipros)
    }
  }
  
  ## validate the base counts
  if(!missing(tumorBAM)){
    annedMutect <- as.matrix(annedMutect)
    annedMutect <- cbind(annedMutect,
                         sample_name = gsub("_.*", "", basename(tumorBAM)),
                         ref_name = gsub("_.*", "", basename(normalBAM)),
                         sample_bam = tumorBAM, ref_bam = normalBAM)
    if(reCount){
      if(!IACSUtil:::hasXTTag(tumorBAM, samtoolPath = samtoolPath)){
        uniqueOnly = FALSE
      }
      if(!missing(normalBAM)){
        if(!IACSUtil:::hasXTTag(normalBAM, samtoolPath = samtoolPath)){
          uniqueOnly = FALSE
        }
      }
      #validated <- try(validateMutations(annedMutect, tumorBAM, normalBAM,
      #    chrom = chrom, pos = start, ref = ref, mutation = alt, sName = sName,
      #    nName = nName, uniqueOnly = uniqueOnly, mapQ = mapQ, cores = cores))
      validated <- try(getVariantBaseCounts(sampleBam = tumorBAM, refBam = normalBAM,
                                            variantMat = as.data.frame(annedMutect, stringAsFactors = FALSE),
                                            chrom = chrom, start = start, ref = ref, alt = alt, uniqueOnly = uniqueOnly,
                                            mapQ = mapQ, cores = cores, samtoolPath = samtoolPath))
      if(class(validated) != "try-error"){
        annedMutect <- validated
        adjustment <- apply(annedMutect, 1, getAdjustedFreq,
                            sep = "_", source = c(sName, nName), alt = alt, ref = ref)
        if(nrow(adjustment) == 2){
          annedMutect <- cbind(annedMutect, t(adjustment))
        }else{
          annedMutect <- cbind(annedMutect, adjustment)
        }
      }
    }
  }
  #colnames(annedMutect) <- tolower(annedMutect)
  write.table(formatMutect(annedMutect, isFile = FALSE), file = annedName,
              sep = "\t",
...
annotate_vcf <- function(x, oprefix, odir, 
                         build, use_uuid, 
                         annovarPath, protocols_f, protocols_g, ...){
...
annotate <- function(x, samplename = get_opts("samplename"), 
                     ...
                     annotate_func = "IACSUtil:::annotate_mutect"){
